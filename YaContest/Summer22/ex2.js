module.exports = function (field, moves) {
    let ans = [];
    let size = 3;
    let snakeCord = [{x:0,y:2}, {x:0,y:1}, {x:0,y:0}]
            ...
            case "R":
                for(let j=0;j<moveD.amount;j++){
                    if(field[snakeCord[0].x][snakeCord[0].y+1]==="-"){
                       snakeCord.unshift({x:snakeCord[0].x,y: snakeCord[0].y+1});
                       snakeCord.pop();
                    } else {
                        snakeCord.unshift({x:snakeCord[0].x,y: snakeCord[0].y+1});
                        size++;
            ...
            case "D":
                for(let j=0;j<moveD.amount;j++){
                    if(field[snakeCord[0].x+1][snakeCord[0].y]==="-"){
                        snakeCord.unshift({x:snakeCord[0].x+1,y: snakeCord[0].y});
                        snakeCord.pop();
                    } else {
                        snakeCord.unshift({x:snakeCord[0].x+1,y: snakeCord[0].y+1});
                        size++;
                    }
                }
                break;
